...
if (!packageJson.scripts?.['validate:public-launch']) {
  errors.push('package.json is missing validate:public-launch.');
}
if (!packageJson.scripts?.['validate:static']?.includes('pnpm validate:public-launch')) {
  errors.push('package.json validate:static must include pnpm validate:public-launch.');
}
