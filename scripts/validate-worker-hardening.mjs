...
for (const required of [
  ...
  'WORKER_TICK_MAX_ATTEMPTS',
  'WORKER_TICK_RETRY_BASE_MS',
  'WORKER_TICK_RETRY_MAX_MS',
...
for (const file of tickFiles) {
  ...
  if (/\bsetInterval\s*\(/.test(source)) {
    errors.push(
      `${relativePath} still schedules its own interval instead of using scheduleWorkerTick.`,
    );
  }

  if (/\.catch\s*\(\s*\(?\s*error\b/.test(source) || /catch\s*\(\s*error\s*\)/.test(source)) {
    errors.push(
      `${relativePath} still has local error handling instead of shared retry/dead-letter handling.`,
    );
  }
}
...
for (const required of [
  ...
  'worker_dead_letters_status_created_idx',
  'worker_dead_letters_tick_created_idx',
]) {
...
if (
  !featureChecklist.includes('Feature Pass 89 - Achievement idempotency and worker dead letters')
) {
