  ...
  for (const contract of routeContracts) {
    ...
    for (const required of contract.requires) {
      if (!source.includes(required)) {
        failures.push(
          `${contract.name}: ${contract.path} is missing required contract token ${required}`,
        );
      }
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(
    `Route contract audit passed: ${result.count} representative route contracts checked.`,
  );
}
