...
Feature flags are stored as `game_config_entries` with keys under `feature.*`. The shared game package owns typed feature definitions and value normalization; the DB package hydrates runtime state; web API route handlers call `requireFeatureEnabled(...)` before high-risk mutations.

Flags are operational kill switches, not authorization. They should block risky writes during incidents, maintenance, abuse spikes, or balance emergencies while keeping read-only routes available for support visibility. Admins can toggle flags from the Admin Console without redeploying.
