import { applyDurabilityScale, combineEquipmentModifiers, normalizeEquipmentModifiers, type EquipmentModifiers } from './equipment';
...
export function calculateVehicleStats(input: VehicleStatInput): EquipmentModifiers {
  ...
  const base = applyDurabilityScale(normalizeEquipmentModifiers(input.vehicleModifiers), durability, maxDurability);
  const upgrades = (input.upgradeModifiers ?? []).map((modifiers) => normalizeEquipmentModifiers(modifiers));
...
export function calculateVehicleTravelPlan(input: VehicleTravelPlanInput) {
  ...
  const effectiveDurationSeconds = Math.max(60, Math.round(input.baseDurationSeconds * durationMultiplier));
  ...
  const riskScore = Math.max(0, input.baseRisk + cargoRisk - travelSafety - heatReduction - smugglingSafety);
...
export function calculateVehicleRepairWear(input: { riskScore: number; cargoQuantity?: number }) {
  return Math.max(1, Math.min(12, 1 + Math.floor(input.riskScore / 3) + Math.ceil((input.cargoQuantity ?? 0) / 10)));
}
