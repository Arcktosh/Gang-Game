...
test('profile, objective, season, and prestige formulas remain stable', () => {
  ...
  assert.deepEqual(
    calculateObjectiveReward({ rewardCash: 99.9, rewardExperience: 15.2, cadence: 'daily' }),
    { cash: 99, experience: 15 },
  );
  ...
  assert.deepEqual(
    calculatePrestigeReset({ legacyPointsAwarded: 10, totalLegacyPoints: 20, prestigeLevel: 1 }),
    {
      cash: 1000,
      bank: 0,
      level: 1,
...
test('admin utility helpers clamp, normalize, and summarize safely', () => {
  ...
  assert.deepEqual(normalizeConfigValue({ market: { enabled: true } }), {
    market: { enabled: true },
  });
