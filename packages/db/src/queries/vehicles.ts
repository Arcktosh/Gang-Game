...
import {
  calculateUpgradeInstallCooldown,
...
function vehicleModifiersFor(
  record: VehicleEquipmentRecord,
  upgrades: InstalledVehicleUpgradeRecord[] = [],
) {
  return calculateVehicleStats({
    vehicleModifiers: normalizeEquipmentModifiers(record.item?.statModifiers),
    upgradeModifiers: upgrades.map((upgrade) =>
...
export async function listVehicleProfile(input: { userId: string; characterId: string }) {
  ...
  const vehicleProfiles = await Promise.all(
    vehicles.map(async (vehicle) => {
      ...
      return {
        ...
        installedUpgrades: installed.map((row) => ({
          id: row.id,
          upgradeKey: row.upgradeKey,
          name: row.upgrade?.name ?? row.upgradeKey,
          upgradeType: row.upgrade?.upgradeType ?? 'engine',
        })),
      };
    }),
  );
...
export async function installVehicleUpgrade(input: {
  userId: string;
  characterId: string;
  equipmentId: string;
  upgradeKey: string;
  ...
  return db.transaction(async (tx) => {
    ...
    if (character.status !== 'free') {
      return {
        ok: false as const,
        code: 'forbidden',
        message: 'Character is not available to install vehicle upgrades.',
      };
    }
    ...
    const upgrade = await tx.query.vehicleUpgradeDefinitions.findFirst({
      where: eq(vehicleUpgradeDefinitions.key, input.upgradeKey),
    });
    ...
    if (character.level < upgrade.requiredLevel) {
      return {
        ok: false as const,
        code: 'forbidden',
        message: `Requires level ${upgrade.requiredLevel}.`,
      };
    }

    const alreadyInstalled = await tx.query.characterVehicleUpgrades.findFirst({
      where: and(
        eq(characterVehicleUpgrades.equipmentId, vehicle.id),
        eq(characterVehicleUpgrades.upgradeKey, upgrade.key),
      ),
    });
    ...
    if (alreadyInstalled) {
      return {
        ok: false as const,
        code: 'forbidden',
        message: 'Upgrade is already installed on this vehicle.',
      };
    }
    ...
    if (character.cash < upgrade.cashCost) {
      return {
        ok: false as const,
        code: 'forbidden',
        message: 'Not enough cash to install this upgrade.',
      };
    }
    ...
    const [installed] = await tx
      .insert(characterVehicleUpgrades)
      .values({ characterId: character.id, equipmentId: vehicle.id, upgradeKey: upgrade.key })
      .returning();
    const [updatedCharacter] = await tx
      .update(characters)
      .set({ cash: character.cash - upgrade.cashCost, updatedAt: sql`now()` })
    ...
    await tx
      ...
      .values({
        ...
        type: 'cash',
        amount: String(-upgrade.cashCost),
        description: `Installed vehicle upgrade: ${upgrade.name}`,
        metadata: { equipmentId: vehicle.id, upgradeKey: upgrade.key },
      });
    await tx
      .insert(playerEvents)
      .values({
        ...
        payload: {
          vehicle: vehicle.item?.name ?? vehicle.itemKey,
          upgrade: upgrade.name,
          cost: upgrade.cashCost,
        },
      });
    await setActionCooldown({
      tx,
      characterId: character.id,
      actionType: 'vehicle_upgrade',
      cooldownSeconds: calculateUpgradeInstallCooldown({ requiredLevel: upgrade.requiredLevel }),
      metadata: { upgradeKey: upgrade.key },
    });
