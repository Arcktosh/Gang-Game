...
export async function getContractEvents(contractId: string) {
  return db.query.contractEvents.findMany({ where: eq(contractEvents.contractId, contractId), orderBy: desc(contractEvents.createdAt), limit: 20 });
}
...
export async function createContract(input: {
  ...
  return db.transaction(async (tx: Tx) => {
    ...
    if (character.status !== 'free') {
      return { ok: false as const, code: 'forbidden', message: 'Character is not available to post contracts.' };
    }
...
export async function acceptContract(input: { userId: string; characterId: string; contractId: string }) {
  return db.transaction(async (tx: Tx) => {
    ...
    if (character.status !== 'free') {
      return { ok: false as const, code: 'forbidden', message: 'Character is not available to accept contracts.' };
    }
    ...
    const contract = await tx.query.contracts.findFirst({ where: eq(contracts.id, input.contractId) });
    ...
    if (contract.expiresAt && new Date(contract.expiresAt).getTime() < Date.now()) {
      await tx.update(contracts).set({ status: 'expired', updatedAt: sql`now()` }).where(eq(contracts.id, contract.id));
      return { ok: false as const, code: 'forbidden', message: 'This contract has expired.' };
    }

    const accepted = await acceptOpenContract(tx, { contractId: contract.id, assigneeCharacterId: character.id });
...
export async function completeContract(input: { userId: string; characterId: string; contractId: string }) {
  return db.transaction(async (tx: Tx) => {
    ...
    if (character.status !== 'free') {
      return { ok: false as const, code: 'forbidden', message: 'Character is not available to complete contracts.' };
    }
    ...
    if (!contract || contract.status !== 'accepted' || contract.assignedToCharacterId !== character.id) {
      return { ok: false as const, code: 'not_found', message: 'Accepted contract not found for this character.' };
    }

    const inventoryItem = contract.itemKey
      ? await tx.query.inventoryItems.findFirst({ where: and(eq(inventoryItems.characterId, character.id), eq(inventoryItems.itemKey, contract.itemKey)) })
      : null;
    ...
    const completed = await completeAcceptedContract(tx, { contractId: contract.id, assigneeCharacterId: character.id });

    if (!completed.ok) {
      return { ok: false as const, code: 'conflict', message: 'This contract is no longer available to complete.' };
    }

    if (contract.itemKey && inventoryItem && contract.quantity > 0) {
      const reservedItem = await decrementInventoryQuantity(tx, inventoryItem.id, contract.quantity);
...
export async function cancelContract(input: { userId: string; characterId: string; contractId: string }) {
  return db.transaction(async (tx: Tx) => {
    ...
    if (contract.status !== 'open') {
      return { ok: false as const, code: 'forbidden', message: 'Only open contracts can be cancelled.' };
    }

    const cancelled = await cancelOpenContract(tx, { contractId: contract.id, creatorCharacterId: character.id });
