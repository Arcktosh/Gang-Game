...
const ASSIGNMENT_TYPES = new Set<ContactAssignmentType>([
  'job_assist',
  'crime_setup',
  'shop_shift',
  'territory_scout',
...
export async function listContactsProfile(input: { userId: string; characterId: string }) {
  ...
  const [definitions, contacts, assignments] = await Promise.all([
    db.query.npcContactDefinitions.findMany({
      orderBy: (table, { asc }) => [asc(table.minLevel), asc(table.name)],
    }),
    db.query.characterContacts.findMany({
      where: eq(characterContacts.characterId, character.id),
      with: { definition: true },
      orderBy: (table, { asc }) => [asc(table.specialty), asc(table.createdAt)],
    }),
    db.query.contactAssignments.findMany({
      where: eq(contactAssignments.characterId, character.id),
      with: { contact: { with: { definition: true } } },
  ...
  const recruitable = definitions.map((definition: any) => ({
    ...
    canRecruit:
      ...
      character.level >= definition.minLevel &&
      character.cash >= definition.recruitCost,
    calculatedRecruitCost: calculateRecruitCost(definition.minLevel, definition.specialty),
  }));

  const assignmentTypes: ContactAssignmentType[] = [
    'job_assist',
    'crime_setup',
    'shop_shift',
    'territory_scout',
...
export async function runContactAction(input: { userId: string } & ContactAction) {
  return db.transaction(async (tx) => {
    ...
    if (input.action === 'recruit') {
      const definition = await tx.query.npcContactDefinitions.findFirst({
        where: eq(npcContactDefinitions.key, input.contactKey),
      });
      if (!definition) {
      ...
      if (character.level < definition.minLevel) {
        return {
          ok: false as const,
          code: 'forbidden',
          message: `Requires level ${definition.minLevel}.`,
        };
      }
      const existing = await tx.query.characterContacts.findFirst({
        where: and(
          eq(characterContacts.characterId, character.id),
          eq(characterContacts.contactKey, definition.key),
        ),
      });
      if (existing) {
      ...
      const cost =
        ...
        calculateRecruitCost(definition.minLevel, definition.specialty as any);
      ...
      const [contact] = await tx
        ...
        .values({
          ...
          upkeep:
            definition.upkeep ||
            calculateContactUpkeep(definition.minLevel, definition.specialty as any),
        })
        .returning();
      ...
      await tx
        ...
        .values({
          ...
          type: 'system',
          amount: String(-cost),
          description: `Recruited ${definition.name}`,
        });
      await tx
        .insert(playerEvents)
        .values({
          userId: input.userId,
          characterId: character.id,
          type: 'contact_recruited',
          visibility: 'private',
          payload: { contactKey: definition.key, cost },
        });
      await setActionCooldown({
        ...
        actionType: 'contacts',
        cooldownSeconds: 90,
        metadata: { action: 'recruit' },
      });
      return { ok: true as const, data: { contact, cost } };
    }

    const contact = await tx.query.characterContacts.findFirst({
      where: and(
        eq(characterContacts.id, input.contactId),
        eq(characterContacts.characterId, character.id),
      ),
      with: { definition: true },
    });
    ...
    if (input.action === 'pay_upkeep') {
      ...
      const [updated] = await tx
        .update(characterContacts)
        .set({ loyalty: nextLoyalty, status: 'idle', updatedAt: sql`now()` })
        .where(eq(characterContacts.id, contact.id))
        .returning();
      await tx
        .update(characters)
        .set({ cash: sql`${characters.cash} - ${contact.upkeep}`, updatedAt: sql`now()` })
        .where(eq(characters.id, character.id));
      await tx
        .insert(financialTransactions)
        .values({
          characterId: character.id,
          type: 'system',
          amount: String(-contact.upkeep),
          description: `Paid contact upkeep`,
        });
      await tx
        .insert(playerEvents)
        .values({
          userId: input.userId,
          characterId: character.id,
          type: 'contact_upkeep_paid',
          visibility: 'private',
          payload: { contactId: contact.id, upkeep: contact.upkeep, loyalty: nextLoyalty },
        });
      await setActionCooldown({
        tx,
        characterId: character.id,
    ...
    const durationSeconds = calculateAssignmentDurationSeconds(
    ...
    await tx
      .update(characterContacts)
      .set({ status: 'assigned', statusUntil: assignment.completesAt, updatedAt: sql`now()` })
      .where(eq(characterContacts.id, contact.id));
    await tx
      .insert(playerEvents)
      .values({
        ...
        visibility: 'private',
        payload: { contactId: contact.id, assignmentType: input.assignmentType, durationSeconds },
      });
    await setActionCooldown({
      tx,
      characterId: character.id,
...
export async function completeReadyContactAssignments() {
  const ready = await db.query.contactAssignments.findMany({
    where: and(
      eq(contactAssignments.status, 'queued'),
      lte(contactAssignments.completesAt, sql`now()` as any),
    ),
    with: { contact: true },
    limit: 50,
  });
  ...
  for (const assignment of ready as any[]) {
    await db.transaction(async (tx) => {
      ...
      if (outcome.success && assignment.rewardCash > 0) {
        await tx
          .update(characters)
          .set({ cash: sql`${characters.cash} + ${assignment.rewardCash}`, updatedAt: sql`now()` })
          .where(eq(characters.id, assignment.characterId));
        await tx
          .insert(financialTransactions)
          .values({
            characterId: assignment.characterId,
            type: 'cash',
            amount: String(assignment.rewardCash),
            description: `Contact assignment completed`,
          });
      }
      ...
      await tx.insert(playerEvents).values({
        ...
        payload: {
          ...
          rewardCash: outcome.success ? assignment.rewardCash : 0,
          loyaltyDelta: outcome.loyaltyDelta,
          experienceGain: outcome.experienceGain,
        },
      });
    });
  }
