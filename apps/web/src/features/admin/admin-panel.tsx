...
export function AdminPanel({
  ...
  async function handleConfig(event: FormEvent<HTMLFormElement>) {
    ...
    await runAdminAction(
      '/api/admin/config',
      'PATCH',
      { key, label, category, description, isPublic, value },
      `Updated ${key}.`,
    );
  }
    ...
    <section style={{ display: 'grid', gap: 24 }}>
      <article className="admin-card">
        ...
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>
          Find a character by name, UUID, player email, or display name before applying flags,
          enforcement, or account adjustments.
        </p>
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <input
            name="query"
            placeholder="Character name, UUID, or email"
            ...
            searchResults.map((character) => (
              <div key={character.id} style={{ borderTop: '1px solid #27272a', paddingTop: 8 }}>
                ...
                <p style={{ color: '#a1a1aa', margin: '4px 0' }}>
                  {character.userEmail} · {character.location} · ${character.cash} cash / $
                  {character.bank} bank
                </p>
                <p style={{ margin: '4px 0' }}>
                  Heat {character.heat} · Reputation {character.reputation} · Active flags{' '}
                  {character.activeFlags} · Active enforcements {character.activeEnforcements}
                </p>
                <code style={{ display: 'block', overflowX: 'auto' }}>{character.id}</code>
                {character.statusReason ? (
                  <p style={{ color: '#facc15', margin: '4px 0' }}>{character.statusReason}</p>
                ) : null}
                {character.status !== 'active' ? (
                  ...
                  >
                    ...
                    <input
                      name="reason"
                      defaultValue="Cleared after admin review."
      ...
      <article className="admin-card">
        ...
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>
          Economy-manager visibility for active, overdue, defaulted, and repaid character loans.
        </p>
        <div
          style={{
            display: 'grid',
        ...
        >
          <p style={{ margin: 0 }}>
            <strong>{loanExposure.summary.activeCount}</strong>
            <br />
            <span style={{ color: '#a1a1aa' }}>active</span>
          </p>
          <p style={{ margin: 0 }}>
            <strong>{loanExposure.summary.overdueCount}</strong>
            <br />
            <span style={{ color: '#a1a1aa' }}>overdue</span>
          </p>
          <p style={{ margin: 0 }}>
            <strong>{loanExposure.summary.defaultedCount}</strong>
            <br />
            <span style={{ color: '#a1a1aa' }}>defaulted</span>
          </p>
          <p style={{ margin: 0 }}>
            <strong>${loanExposure.summary.unresolvedOutstanding}</strong>
            <br />
            <span style={{ color: '#a1a1aa' }}>unresolved outstanding</span>
          </p>
        </div>
        <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
          {loanExposure.loans.length ? (
            loanExposure.loans.map((loan) => (
              <div key={loan.id} style={{ borderTop: '1px solid #27272a', paddingTop: 10 }}>
                ...
                <p style={{ color: '#a1a1aa', margin: '4px 0' }}>
                  {loan.userEmail} · Level {loan.level} · Heat {loan.heat} · {loan.characterStatus}
                </p>
                <p style={{ margin: '4px 0' }}>
                  ${loan.outstanding} outstanding of ${loan.totalDue} · paid ${loan.repaidAmount} ·
                  bank ${loan.bank}
                </p>
                <p
                  style={{
                    color:
                      loan.lifecycleStatus === 'defaulted'
                        ? '#f87171'
                        : loan.lifecycleStatus === 'overdue'
                          ? '#facc15'
                          : '#a1a1aa',
                    margin: '4px 0',
                  }}
                >
                  Due {formatDateTime(loan.dueAt)}
                  {loan.hoursPastDue > 0 ? ` · ${loan.hoursPastDue}h past due` : ''}
                </p>
      ...
      >
        ...
        <article className="admin-card">
          ...
          <form onSubmit={handleEnforcement} style={{ display: 'grid', gap: 8 }}>
            ...
            <input
              ...
              min="1"
              max="720"
              placeholder="Duration hours for temporary actions"
            />
        ...
        <article className="admin-card">
          ...
          <form onSubmit={handleAdjust} style={{ display: 'grid', gap: 8 }}>
            <input name="characterId" required placeholder="Character UUID" />
            <select name="wallet" defaultValue="cash">
      ...
      <article className="admin-card">
        ...
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>
          Last {transparency.days} days, aggregated without exposing private report content.
        </p>
        <div
          style={{
            display: 'grid',
        ...
        >
          <div>
            ...
            {transparency.reports.length ? (
              transparency.reports.map((entry) => (
                <p key={`${entry.kind}-${entry.status}`} style={{ margin: '4px 0' }}>
                  {entry.kind} / {entry.status}: {entry.count}
                </p>
              ))
            ) : (
              <p>No reports.</p>
          ...
          <div>
            ...
            {transparency.enforcements.length ? (
              transparency.enforcements.map((entry) => (
                <p key={entry.actionType} style={{ margin: '4px 0' }}>
                  {entry.actionType}: {entry.count}
                </p>
              ))
            ) : (
              <p>No enforcements.</p>
          ...
          <div>
            ...
            {transparency.appeals.length ? (
              transparency.appeals.map((entry) => (
                <p key={entry.status} style={{ margin: '4px 0' }}>
      ...
      <article className="admin-card">
        ...
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>
          Open reports: {moderation.messages.length + moderation.articles.length}. Review, dismiss,
          or action reports without touching the database directly.
        </p>
        <div
          style={{
            display: 'grid',
        ...
        >
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            {moderation.messages.length ? (
              moderation.messages.map((report) => (
                <form
                  key={report.id}
                ...
                >
                  ...
                  <strong>
                    {report.senderName} reported by {report.reporterName}
                  </strong>
                  <p style={{ color: '#a1a1aa', margin: 0 }}>
                    {formatDateTime(report.createdAt)} · {report.reason || 'No reason provided.'}
                  </p>
                  <blockquote
                    style={{ borderLeft: '3px solid #52525b', margin: 0, paddingLeft: 12 }}
                  >
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            {moderation.articles.length ? (
              moderation.articles.map((report) => (
                <form
                  key={report.id}
                  onSubmit={handleModeration}
                  style={{
                ...
                >
                  ...
                  <p style={{ color: '#a1a1aa', margin: 0 }}>
                    Author: {report.authorName ?? 'System'} · Reporter: {report.reporterName} ·{' '}
                  ...
                  <label style={{ display: 'flex', gap: 8 }}>
                    <input name="hideArticle" type="checkbox" /> Unpublish article
                  </label>
                  <button disabled={submitting} type="submit">
                    Resolve article report
                  </button>
                </form>
              ))
            ) : (
      ...
      <article className="admin-card">
        ...
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>
          Recently reviewed, dismissed, and actioned reports for moderation history audits.
        </p>
        <div
          style={{
            display: 'grid',
        ...
        >
          {moderationArchive.map((bucket) => (
            <div key={bucket.status}>
              ...
              {bucket.messages.length + bucket.articles.length === 0 ? (
                <p>No {bucket.status} reports.</p>
              ) : null}
            </div>
          ))}
        </div>
      ...
      >
        <article className="admin-card">
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {audit.activeEnforcements?.length ? (
              audit.activeEnforcements.map((enforcement) => (
                <form
                  key={enforcement.id}
                  onSubmit={handleLiftEnforcement}
                  style={{
                    borderTop: '1px solid #27272a',
                    display: 'grid',
                    gap: 8,
                ...
                >
                  ...
                  <strong>
                    {enforcement.characterName} · {enforcement.actionType}
                  </strong>
        ...
        <article className="admin-card">
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {audit.openAppeals?.length ? (
              audit.openAppeals.map((appeal) => (
                <form
                  key={appeal.id}
                  onSubmit={handleAppealReview}
                  style={{
                    borderTop: '1px solid #27272a',
                    display: 'grid',
                    gap: 8,
                ...
                >
                  ...
                  <strong>
                    {appeal.characterName} · {appeal.actionType}
                  </strong>
                  ...
                  <label style={{ display: 'flex', gap: 8 }}>
                    <input name="liftEnforcement" type="checkbox" /> Lift enforcement when accepted
                  </label>
                  <button disabled={submitting} type="submit">
                    Review appeal
                  </button>
      ...
      >
        <article className="admin-card">
          ...
          <p style={{ color: '#a1a1aa', marginTop: 0 }}>
            Recent sessions with IP and user-agent visibility for account-abuse triage.
          </p>
          <div style={{ display: 'grid', gap: 8 }}>
            {audit.recentSessions?.length ? (
              audit.recentSessions.map((session) => (
                <div key={session.id} style={{ borderTop: '1px solid #27272a', paddingTop: 8 }}>
                  <strong>{session.email}</strong>
                  <p style={{ color: '#a1a1aa', margin: '4px 0' }}>
                    {session.displayName ?? 'No display name'} · Last seen{' '}
                    {formatDateTime(session.lastSeenAt)}
                  </p>
                  <p style={{ margin: '4px 0' }}>
                    IP {session.ipAddress ?? 'unknown'} · Expires{' '}
        ...
        <article className="admin-card">
          ...
          <p style={{ color: '#a1a1aa', marginTop: 0 }}>
            Highest current item quantities for spotting suspicious inventory spikes.
          </p>
          <div style={{ display: 'grid', gap: 8 }}>
            {audit.inventoryHighlights?.length ? (
              audit.inventoryHighlights.map((item) => (
                <div key={item.id} style={{ borderTop: '1px solid #27272a', paddingTop: 8 }}>
                  <strong>{item.characterName}</strong> · {item.itemKey}
                  <p style={{ color: '#a1a1aa', margin: '4px 0' }}>
                    Quantity {item.quantity} · Updated {formatDateTime(item.updatedAt)}
                  </p>
      ...
      >
        <article className="admin-card">
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {audit.activeFlags.length ? (
              audit.activeFlags.map((flag) => (
                <form
                  key={flag.id}
        ...
        <article className="admin-card">
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {audit.adminLogs.length ? (
              audit.adminLogs.map((entry) => (
                <div key={entry.id}>
