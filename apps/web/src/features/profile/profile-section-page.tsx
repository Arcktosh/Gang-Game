...
const profileSectionConfig: Record<
  ProfileSection,
...
> = {
  overview: {
    ...
    eyebrow: 'Profile',
    description:
      'Review your character resources, progression, public profile signals, and privacy boundaries.',
  },
  status: {
    title: 'Character status',
    eyebrow: 'Profile',
    description: 'Review legal, medical, location, and public-profile status information.',
  },
  titles: {
    title: 'Character titles',
    eyebrow: 'Profile',
    description: 'Review earned titles and choose the title displayed on your public profile.',
  },
  rewards: {
    title: 'Claimable rewards',
    eyebrow: 'Profile',
    description: 'Claim completed achievement and objective rewards.',
  },
  achievements: {
    title: 'Achievements',
    eyebrow: 'Profile',
    description: 'Track recent achievement completion and progress.',
  },
  history: {
    title: 'Character history',
    eyebrow: 'Profile',
    description: 'Review recent public, private, and administrative character events.',
  },
};

export async function ProfileSectionPage({ section }: { section: ProfileSection }) {
  ...
  const config = profileSectionConfig[section];
  ...
  return (
    <GamePageShell
      ...
      title={section === 'overview' ? `${character.name} profile` : config.title}
      eyebrow={config.eyebrow}
      description={config.description}
    >
      {section === 'overview' ? (
        <Grid>
          <CollapsibleCard title="Resources" defaultOpen>
          ...
          <CollapsibleCard title="Progression">
            <div className="progress-grid">
              <ProfileProgressMeter
          ...
          <CollapsibleCard title="Public profile mode">
      ...
      {section === 'status' ? (
        <Grid>
          <CollapsibleCard title="Legal and medical status" defaultOpen>
            <StatList
              items={[
          ...
          <CollapsibleCard title="Public privacy boundary">
            <p style={{ color: '#a1a1aa', marginTop: 0 }}>
      ...
      {section === 'titles' ? (
        <Grid>
          <CollapsibleCard
            ...
            meta={`${progression.titles.length} earned`}
            defaultOpen
          >
      ...
      {section === 'rewards' ? (
        <Grid>
          ...
          <CollapsibleCard
            title="Claimable objective rewards"
            meta={`${claimableObjectives.length} shown`}
          >
      ...
      {section === 'achievements' ? (
        <Grid>
          <CollapsibleCard
            ...
            meta={`${recentAchievements.length} shown`}
            defaultOpen
          >
      ...
      {section === 'history' ? (
        <Grid>
          <CollapsibleCard title="Recent event history" meta={`${events.length} shown`} defaultOpen>
            {events.length > 0 ? (
              <div style={{ display: 'grid', gap: 10 }}>
                {events.map((event) => (
                  <article
