...
import { db, inventoryItems, listActiveActionLocks, listActiveShopListings, listShops, listShopsForCharacter } from '@drugdeal/db';
...
export default async function ShopsPage() {
  ...
  const [shops, ownShops, listings, inventory, actionLocks] = await Promise.all([
    ...
    db.query.inventoryItems.findMany({ where: eq(inventoryItems.characterId, character.id), orderBy: (item, { asc }) => [asc(item.itemKey)] }),
    ...
    >
          ...
          {purchasableListings.length > 0 ? (
            <div style={{ display: 'grid', gap: 10 }}>
              {purchasableListings.map((listing) => {
                ...
                return (
                  <article key={listing.listingId} style={{ borderTop: '1px solid #27272a', paddingTop: 10 }}>
                    <strong>{listing.itemName}</strong>
                    <p style={{ color: '#a1a1aa', margin: '4px 0 0' }}>
                      {listing.shopName} - {remaining} left at {money(listing.priceEach)} each · {listing.itemCategory}
                    </p>
                    <GameActionForm
      ...
      <Card title="Your shop operations" meta={`${ownShops.length} storefronts`}>
        {ownShops.length > 0 ? (
              ...
              <article key={entry.shop.id} style={{ borderTop: '1px solid #27272a', paddingTop: 14 }}>
                ...
                <Grid min={300}>
                    ...
                    {entry.listings.length > 0 ? (
                      <div style={{ display: 'grid', gap: 8 }}>
                        {entry.listings.map((listing) => (
                          <div key={listing.id} style={{ borderTop: '1px solid #27272a', paddingTop: 8 }}>
                            <strong>{listing.itemName}</strong>
                  ...
                  <div>
                    ...
                    {entry.activeAds.length ? <p>{entry.activeAds.length} active campaign(s), next ending {formatDate(entry.activeAds[0]?.endsAt)}.</p> : <p>No active ad campaigns.</p>}
          ...
          <EmptyState>You do not own a shop yet. Use the shop opening form above to create your first storefront.</EmptyState>
        )}
      </Card>
    </GamePageShell>
  );
