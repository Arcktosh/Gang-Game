...
export default async function LegalPage() {
  ...
  return (
    <GamePageShell sidebarCharacter={character} title="Legal and hospital" eyebrow={character.name} description="Track jail, hospital, lawyer, bribe, court, bail, and recovery state from one MVP status page.">
      <Grid>
        ...
        <Card title="Active penalties">
          <StatList items={[
            ...
            { label: 'Estimated fine settlement', value: fineSettlement ? money(fineSettlement.cost) : money(0) },
            { label: 'Estimated bail', value: bailSettlement ? money(bailSettlement.cost) : money(0) },
      ...
      <Card title="Jail resolution">
        <p style={{ color: '#a1a1aa', marginTop: 0 }}>Settle an active sentence through abstract, fictional MVP actions. Court and jail activities use cooldowns to prevent repeat-spam.</p>
        <Grid>
          <GameActionForm
            endpoint="/api/legal/jail"
            label="Pay fine"
            payload={{ characterId: character.id, action: 'pay_fine' }}
            fields={[{ name: 'paymentSource', label: 'Payment source', type: 'select', options: [
              { label: 'Cash', value: 'cash' },
              { label: 'Bank', value: 'bank' },
            ] }]}
            helper={fineSettlement ? `Estimated cost: ${money(fineSettlement.cost)}.` : 'No active fine.'}
          ...
          <GameActionForm
            ...
            helper={bailSettlement ? `Estimated cost: ${money(bailSettlement.cost)}.` : 'No active bail offer.'}
          ...
          <GameActionForm
            ...
            payload={{ characterId: character.id }}
            fields={[{ name: 'plea', label: 'Approach', type: 'select', options: [
          ...
          <GameActionForm
            ...
            fields={[{ name: 'activity', label: 'Activity', type: 'select', options: [
