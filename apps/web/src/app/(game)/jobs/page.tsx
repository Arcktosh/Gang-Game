...
export default async function JobsPage() {
  ...
  return (
    <GamePageShell
      ...
      description="Apply for a legal job, work shifts for cash and XP, earn promotions, or resign before switching careers."
    >
      ...
      <p style={{ color: '#a1a1aa', marginTop: 0 }}>
        Available lifecycle actions: <code>apply</code>, <code>work</code>, and <code>resign</code>.
      </p>
      {jobs.length > 0 ? (
        <Grid>
          {jobs.map((job) => {
            ...
            return (
              <Card
                key={job.key}
                title={job.name}
                meta={isActive ? 'Current job' : `${job.durationSeconds}s cooldown`}
              >
                ...
                {isActive ? (
                  <>
                    <GameActionForm
                      endpoint="/api/jobs"
                      label="Work shift"
                      payload={{ characterId: character.id, jobKey: job.key, action: 'work' }}
                      successMessage="Shift completed."
                      cooldown={jobCooldown}
                    />
                    <GameActionForm
                      endpoint="/api/jobs"
                      label="Resign"
                      payload={{ characterId: character.id, jobKey: job.key, action: 'resign' }}
                      successMessage="You resigned from this job."
                    />
                  </>
                ) : (
                  <GameActionForm
                    endpoint="/api/jobs"
                    label="Apply"
                    payload={{ characterId: character.id, jobKey: job.key, action: 'apply' }}
        ...
        <Card>
          <EmptyState>No jobs are configured yet.</EmptyState>
        </Card>
      )}
    </GamePageShell>
  );
