...
const inventorySectionItems = [
  { label: 'Summary', href: '/inventory#inventory-summary', icon: '▣' },
  { label: 'Items', href: '/inventory#inventory-items', icon: '◈' },
...
const focusedInventorySections = inventorySectionItems.map((section) => ({
...
export default async function InventoryPage() {
  ...
  if (!profileResult.ok) {
    return (
      <GamePageShell
        ...
        title="Inventory"
        eyebrow="Items"
        description="Manage consumables, item risk, and direct same-location transfers."
      >
        <Card title="Inventory unavailable">
  ...
  return (
    <GamePageShell
      sidebarCharacter={character}
      sectionItems={inventorySectionItems}
      title="Inventory"
      eyebrow={character.location}
      description="Review item value, risk, rarity, consumables, and safe same-location hand-offs."
    >
      <FocusedSections sections={focusedInventorySections} label="inventory section">
        <section id="inventory-summary" className="dashboard-section">
          <Grid>
            ...
            <Card title="Consumable readiness" meta={`${consumables.length} stacks`}>
            ...
            <Card title="Risk review" meta={`${highRiskItems.length} stacks`}>
        ...
        <section id="inventory-items" className="dashboard-section">
          <Grid>
            {inventory.length > 0 ? (
              inventory.map((row) => (
                <Card
        ...
        <section id="inventory-transfers" className="dashboard-section">
          <Grid>
            <Card title="Transfer rules">
              <p style={{ color: '#a1a1aa', marginTop: 0 }}>
            ...
            {inventory.length > 0 ? (
              ...
              <Card title="No transferable inventory">
