...
export default async function CrimesPage() {
  ...
  return (
    <GamePageShell
      ...
      description="Attempt abstract fictional crimes, balancing reward against heat, jail, and hospital consequences."
    >
      {availableCrimes.length > 0 ? (
            ...
            <Card key={crime.key} title={crime.name} meta={`Difficulty ${crime.difficulty}`}>
              ...
              <StatList
                items={[
                  ...
                  {
                    label: 'Reward',
                    value: `${money(crime.minReward)} - ${money(crime.maxReward)}`,
                  },
              ...
              <GameActionForm
                endpoint="/api/crimes"
                label="Attempt crime"
                payload={{ characterId: character.id, crimeKey: crime.key }}
                successMessage="Crime attempt resolved."
                cooldown={getActionCooldown(actionLocks, 'crime')}
              />
            </Card>
          ))}
        </Grid>
        ...
        <Card>
          <EmptyState>
            No crimes are available at your current level. Level-gated actions stay hidden until
            unlocked.
          </EmptyState>
        </Card>
      )}
