...
export default async function MessagesPage() {
  ...
  return (
    <GamePageShell
      ...
      description="Review private conversations, live inbox updates, social safety controls, reports, and mutes."
    >
      <Grid>
        <MessageLivePanel
          characterId={character.id}
          initialThreadCount={threads.length}
          initialUnreadTotal={center?.unreadTotal ?? 0}
        />
                ...
                >
                  <div>
                    ...
                    <p style={{ color: '#a1a1aa', margin: '4px 0 0' }}>
                      {thread.members.map((member) => member.name).join(', ')} -{' '}
                      {thread.unreadCount} unread - Updated {formatDate(thread.thread?.createdAt)}
                    </p>
                  </div>
                  <div
                    className="action-grid"
                    aria-label={`Actions for ${thread.thread?.title || 'conversation'}`}
                  >
                    <GameActionForm
        ...
        <Card
          ...
          meta={`${threads.reduce((total, thread) => total + thread.recentMessages.length, 0)} shown`}
        >
                        ...
                        fields={[
                          {
                            name: 'reason',
                            label: 'Reason',
                            placeholder: 'Why should moderators review this message?',
                            omitWhenEmpty: true,
                          },
                        ]}
              ...
              recipients.length > 0
                ? {
                    ...
                    options: recipients.map((recipient) => ({
                      label: `${recipient.name} · level ${recipient.level} · ${recipient.location}`,
                      value: recipient.id,
                    })),
                  }
                : {
                    name: 'recipientCharacterId',
                    label: 'Recipient character ID',
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {recipients.length > 0 ? (
              <GameActionForm
                ...
                fields={[
                  {
                    name: 'blockedCharacterId',
                    label: 'Character',
                    type: 'select',
                    options: recipients.map((recipient) => ({
                      label: `${recipient.name} · level ${recipient.level}`,
                      value: recipient.id,
                    })),
                  },
                  {
                ...
                {blocked.map((block) => (
                  ...
                  >
                    ...
                    <p style={{ color: '#a1a1aa', margin: '4px 0 8px' }}>
                      {block.reason || 'No reason recorded'} - blocked {formatDate(block.createdAt)}
                    </p>
                    <GameActionForm
                      endpoint="/api/messages"
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {reports.map((report) => (
                <article key={report.id} style={{ borderTop: '1px solid #27272a', paddingTop: 10 }}>
                  ...
                  <p style={{ color: '#a1a1aa', margin: '4px 0 0' }}>
                    {report.reason || 'No reason recorded'} - {formatDate(report.createdAt)}
                  </p>
                </article>
              ))}
            </div>
