import { acceptContract } from '@drugdeal/db';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ contractId: string }> }) {
  return withApiObservability(request, async () => {
    ...
    const limit = await assertRateLimit({ key: rateLimitKey(request, 'api:contracts:id:accept', auth.userId), windowSeconds: 60, maxRequests: 30 });
    ...
    return withIdempotency({
      request,
      userId: auth.userId,
      routeScope: 'contracts:accept',
      fingerprint: { ...body.data, contractId },
      handler: async () => {
        const result = await acceptContract({ userId: auth.userId, characterId: body.data.characterId, contractId });
