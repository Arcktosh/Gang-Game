...
export async function POST(
  ...
  return withApiObservability(request, async () => {
    ...
    const limit = await assertRateLimit({
      key: rateLimitKey(request, 'api:admin:characters:id:flag', session.user.id),
      windowSeconds: 60,
      maxRequests: 30,
    });
    ...
    try {
      ...
      const flag = await addCharacterFlag({
      ...
      return jsonError(
        'bad_request',
        caught instanceof Error ? caught.message : 'Could not flag character.',
        400,
      );
    }
  });
