...
export async function POST(
  ...
  return withApiObservability(request, async () => {
    ...
    const limit = await assertRateLimit({
      key: rateLimitKey(request, 'api:admin:characters:id:enforce', session.user.id),
      windowSeconds: 60,
      maxRequests: 30,
    });
    ...
    return withIdempotency({
      ...
      handler: async () => {
        try {
          const result = await applyCharacterEnforcement({
          ...
          return jsonError(
            'bad_request',
            caught instanceof Error ? caught.message : 'Could not apply enforcement.',
            400,
          );
        }
      },
