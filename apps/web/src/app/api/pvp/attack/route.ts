...
export async function POST(request: NextRequest) {
  return withApiObservability(request, async () => {
    ...
    const limit = await assertRateLimit({ key: rateLimitKey(request, 'api:pvp:attack', auth.userId), windowSeconds: 60, maxRequests: 30 });

    if (!limit.ok) {
      return limit.response;
    }

    const feature = await requireFeatureEnabled('feature.pvp');
