import { factionMemberRoleSchema } from '@drugdeal/validators';
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ factionId: string }> }) {
  return withApiObservability(request, async () => {
    ...
    const result = await setFactionMemberRole({ userId: auth.userId, factionId, ...body.data });

    if (!result.ok) {
      return jsonError(result.code, result.message, result.code === 'not_found' ? 404 : 403);
    }

    return jsonOk({ membership: result.membership });
  });
}
