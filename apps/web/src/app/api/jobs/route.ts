...
import { calculateActionExperience, calculateJobPayout, calculateProgressionFromExperience } from '@drugdeal/game';
    ...
    return withIdempotency({
      ...
      handler: async () => {
        const result = await db.transaction(async (tx) => {
          ...
          const job = await tx.query.jobDefinitions.findFirst({ where: eq(jobDefinitions.key, body.data.jobKey) });
          ...
          if (refreshedCharacter.labour < job.requiredLabour || refreshedCharacter.intelligence < job.requiredIntelligence) {
            return { error: jsonError('forbidden', 'Character does not meet the job requirements.', 403) };
          }
          ...
          if (!activeEmployment || activeEmployment.jobKey !== job.key) {
            return { error: jsonError('forbidden', 'Apply for this job before working or resigning.', 403) };
          }
          ...
          const experienceGain = calculateActionExperience({ base: Math.max(1, Math.floor(payout / 10)), difficulty: job.requiredLabour + job.requiredIntelligence + activeEmployment.rank, success: true });
        
          const [jobRun] = await tx
            .insert(jobRuns)
            .values({ characterId: refreshedCharacter.id, jobKey: job.key, payout, completedAt: sql`now()` })
            .returning();
          ...
          await tx.insert(playerEvents).values({
            ...
            payload: { jobKey: job.key, jobName: job.name, payout, energyCost: job.energyCost, experienceGain, rank: employment.rank, shiftsCompleted: employment.shiftsCompleted, progression: calculateProgressionFromExperience(updatedCharacter.experience) },
          });
          ...
          return { data: { jobRun, employment, character: updatedCharacter, progression: calculateProgressionFromExperience(updatedCharacter.experience), lock } };
        });
