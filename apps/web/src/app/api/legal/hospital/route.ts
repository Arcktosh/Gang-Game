...
export async function POST(request: NextRequest) {
  return withApiObservability(request, async () => {
    ...
    const limit = await assertRateLimit({ key: rateLimitKey(request, 'api:legal:hospital', auth.userId), windowSeconds: 60, maxRequests: 30 });
